...
def main():
    ...
    """
    #명령어의 첫 번째 매개변수로
    #WikiExtractor의 출력 디렉터리를 지정한다.
    input_dir=sys.argv[1]
    kkma=Kkma()

    #단어의 빈도를 저장하기 위한 Counter객체를 생성한다.
    #Counter클래스는 dict를 상속받는 클래스이다.
    frequency=Counter()
    count_proccessed=0
...
def iter_docs(file):
    ...
    for line in file:
        if line.startswith("<doc "):
            #시작 태그가 찾아지면 버퍼를 초기화한다.
            buffer=[]
        elif line.startswith("</doc>"):
            #종료 태그가 찾아지면 버퍼의 내용을 결합한 뒤 yield한다.
            content=''.join(buffer)
            yield content
        else:
            #시작 태그/종료 태그 이외의 줄은 버퍼에 추가한다.
            buffer.append(line)
