...
def main():
    #1981년과 2014년 사이의 환율과 고용률을 출력해 본다.
    #조금 이해하기 쉽게 Pandas 대신 기본 숫자 비교와 문자열 비교를 사용한다.

    #환융 정보 읽어 들이기
    df_exchange=pd.read_csv(
        "/mnt/hgfs/share/DEXKOUS.csv",
        header=1,
        names=["DATE","DEXKOUS"],
    ...
    plt.xlim(1981, 2014) #X축의 범위를 설정한다.
    plt.ylim(500, 2500)
    plt.legend(loc="best")
    #두 번째 그래프 그리기
    print(s_jobs)
    ...
    plt.xlim(1981, 2014) #X축의 범위를 설정한다.
    plt.ylim(0, 100)#Y축의 범위를 설정한다.
    plt.legend(loc="best")
    plt.savefig("historical_data.png", dpi=300) #이미지를 저장한다.
